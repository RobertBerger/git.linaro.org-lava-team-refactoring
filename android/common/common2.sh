...
func_loop_apps_for_times(){
    ...
    for apk in ${loop_apps_list}; do
        ...
        while [ ${loop_count} -lt ${loop_apps_times} ]; do
            ...
            # LAVA V2 only support
            # needs a check for LAVA here
            lava-test-set start ${loop_app_package}_${loop_count}
            ...
            # LAVA V2 only support
            # needs a check for LAVA here
            lava-test-set stop ${loop_app_package}_${loop_count}
...
output_test_result(){
    ...
    if [ -z "${measurement}" ]; then
        ...
        ${LAVA} ${test_name} --result ${result}
        lava_paras="${test_name} --result ${result}"
    else
        output="${test_name}=${measurement} ${units}"
        ${LAVA} ${test_name} --result ${result} --measurement ${measurement} --units ${units}
